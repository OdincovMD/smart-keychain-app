...
void main() {
  test('migrates v1 to v3 without losing app settings', () async {
...
''')
  ...
  test('migrates v2 appearance to system and preserves prior values', () async {
    final executor = NativeDatabase.memory(
      setup: (database) {
        database
          ..execute('''
...
INSERT INTO app_settings (
  id, created_at_utc_ms, updated_at_utc_ms, row_revision,
  is_deleted, deleted_at_utc_ms, active_scene_id, brightness_permille
) VALUES ('app', 1, 1, 0, 0, NULL, 'living_eyes_v1', 610)
''')
    ...
    expect(settings.activeSceneId, 'living_eyes_v1');
    expect(settings.brightness, closeTo(0.61, 0.0001));
    expect(settings.appearance, AppAppearance.system);
  });
}
